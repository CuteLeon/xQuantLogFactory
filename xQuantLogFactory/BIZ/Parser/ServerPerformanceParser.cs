...
namespace xQuantLogFactory.BIZ.Parser
{
    ...
    public class ServerPerformanceParser : PerformanceLogParserBase
    {
        ...
        public override Regex GeneralLogRegex { get; } = new Regex(
            @"^(?<LogTime>\d{4}-\d{1,2}-\d{1,2}\s\d{2}:\d{2}:\d{2}.\d{3})\s(?<IPAddress>\d{1,3}(\.\d{1,3}){3})\s(?<UserCode>.*?)\s(?<RequestReceiveTime>\d{4}-\d{1,2}-\d{1,2}\s\d{2}:\d{2}:\d{2}.\d{3})\s(?<ResponseSendTime>\d{4}-\d{1,2}-\d{1,2}\s\d{2}:\d{2}:\d{2}.\d{3})\s(?<Elapsed>\d*?)\s(?<RequestURI>\/.*?)\s(?<MethodName>.*?)\s(?<RequestStreamLength>\d*?)\s(?<ResponseStreamLength>\d*?)\s(?<Message>.+)$",
        ...
        protected override IEnumerable<PerformanceLogFile> GetFileFiltered(TaskArgument argument)
            => argument?.PerformanceLogFiles.Where(file => file.LogFileType == LogFileTypes.Server);
