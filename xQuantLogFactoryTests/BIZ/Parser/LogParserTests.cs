...
namespace xQuantLogFactory.BIZ.Parser.Tests
{
    ...
    public class LogParserTests
    {
        ...
        public void ParseTest()
        {
            ...
            Assert.IsFalse(serverParser.ParticularRegex.IsMatch(IgoreLog));
            Assert.IsFalse(serverParser.ParticularRegex.IsMatch(MiddlewareLog));
            Assert.IsTrue(serverParser.ParticularRegex.IsMatch(ClientLog));

            LogParserBase clientParser = new ClientLogParser();
            Assert.IsFalse(clientParser.ParticularRegex.IsMatch(IgoreLog));
            Assert.IsFalse(clientParser.ParticularRegex.IsMatch(MiddlewareLog));
            Assert.IsFalse(clientParser.ParticularRegex.IsMatch(ServerLog));

            LogParserBase middlewareParser = new MiddlewareLogParser();
            Assert.IsFalse(middlewareParser.ParticularRegex.IsMatch(IgoreLog));
            Assert.IsFalse(middlewareParser.ParticularRegex.IsMatch(ServerLog));
            Assert.IsFalse(middlewareParser.ParticularRegex.IsMatch(ClientLog));

            Match serverMatch = serverParser.ParticularRegex.Match(ServerLog);
            ...
            Match clientMatch = clientParser.ParticularRegex.Match(ClientLog);
