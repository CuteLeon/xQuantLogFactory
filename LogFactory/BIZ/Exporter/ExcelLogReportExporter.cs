...
using LogFactory.Model.EqualityComparer;
using LogFactory.Model.Fixed;
using LogFactory.Model.Report;
        ...
        public void ExportReport(string reportPath, TaskArgument argument)
        {
            ...
            using (ExcelPackage excel = new ExcelPackage(new FileInfo(reportPath)))
            {
                try
                {
                    ...
                    properties.Author = $"日志分析工具 - {Application.ProductVersion}";
                    properties.Category = "日志分析报告";
                    properties.Title = $"日志分析报告-{argument.TaskID}";
